...
function openAddExpenseModal(){
  ...
  openModal('Add spend', html, (body)=>{
    body.querySelector('#fExpSubmit').addEventListener('click', ()=>{
      const amount = Number(body.querySelector('#fExpAmount').value);
      if(!amount || amount<=0){ toast('Enter a valid amount'); return; }
      const [kind,id] = body.querySelector('#fExpPaidVia').value.split(':');
      const ref = { kind, id };
      const t = {
        id: uid(), type:'expense', amount,
        category: body.querySelector('#fExpCategory').value,
        description: body.querySelector('#fExpDesc').value.trim(),
        date: body.querySelector('#fExpDate').value || todayISO(),
...
function renderSplitPage(){
  ...
  document.getElementById('splitList').innerHTML = state.splitGroups.length ? [...state.splitGroups].reverse().map(g=>{
    ...
    <div class="item-card">
      <div class="ic-top">
        <div><div class="ic-name">${escapeHtml(g.title)}</div><div class="ic-sub">Paid ${formatCurrency(g.total)} from ${escapeHtml(payerLabel({kind:'account',id:g.paidFrom}))} · ${formatDateShort(g.date)}</div></div>
  ...
  }).join('') : emptyNote('No split groups yet. Add one when you cover a bill for friends.');
...
document.getElementById('openAddSplit').addEventListener('click', openAddSplitModal);
function openAddSplitModal(){
  if(state.accounts.length===0){ toast('Add a bank account first'); return; }
  const html = `
    <div class="field"><label>What was this for?</label><input id="fSplitTitle" placeholder="e.g. Dinner at ECR"></div>
    <div class="field-row">
      <div class="field"><label>Total amount you paid</label><input type="number" id="fSplitTotal" placeholder="0"></div>
      <div class="field"><label>Paid from</label><select id="fSplitFrom">${accountOptionsOnly()}</select></div>
    </div>
    <div class="field"><label>Date</label><input type="date" id="fSplitDate" value="${todayISO()}"></div>
    <div class="field">
      <label>Friends (their equal share is calculated automatically — you can adjust it)</label>
      <div id="peopleRows"></div>
      <button class="link-btn" id="addPersonRow" type="button">+ Add a friend</button>
    </div>
    <button class="btn-block" id="fSplitSubmit">Create split</button>
  `;
  openModal('New split', html, (body)=>{
    const rows = body.querySelector('#peopleRows');
    function addRow(){
      const row = document.createElement('div');
      row.className = 'people-input-row';
      row.innerHTML = `<input type="text" placeholder="Name" class="pName"><input type="text" placeholder="Phone (optional)" class="pPhone" style="max-width:120px;"><button type="button" class="removeRow">✕</button>`;
      row.querySelector('.removeRow').addEventListener('click', ()=>row.remove());
      rows.appendChild(row);
    }
    addRow();
    body.querySelector('#addPersonRow').addEventListener('click', addRow);

    body.querySelector('#fSplitSubmit').addEventListener('click', ()=>{
      const title = body.querySelector('#fSplitTitle').value.trim();
      const total = Number(body.querySelector('#fSplitTotal').value);
      if(!title){ toast('Add a short title'); return; }
      if(!total || total<=0){ toast('Enter a valid total amount'); return; }
      const names = Array.from(rows.querySelectorAll('.people-input-row')).map(r=>({
        name: r.querySelector('.pName').value.trim(),
        phone: r.querySelector('.pPhone').value.trim()
      })).filter(p=>p.name);
      if(names.length===0){ toast('Add at least one friend'); return; }
      const share = Math.round((total/(names.length+1))*100)/100;
      const accId = body.querySelector('#fSplitFrom').value;
      const group = {
        id: uid(), title, total, paidFrom: accId, date: body.querySelector('#fSplitDate').value || todayISO(),
        people: names.map(n=>({ id: uid(), name:n.name, phone:n.phone, share, paid:false }))
      };
      state.splitGroups.push(group);
      const acc = getAccount(accId); acc.balance -= total;
      state.transactions.push({
        id: uid(), type:'expense', amount: total, category:'Shared Expense',
        description: title, date: group.date, ref:{kind:'account',id:accId}, sourceOrNote: acc.name
      });
      saveState(); closeModal(); toast('Split created'); renderSplitPage();
    });
  });
}
  ...
  state.recurring.forEach(r=>{
    ...
    if(r.type==='expense'){
      ...
      state.transactions.push({
        ...
        ref:r.ref, sourceOrNote: payerLabel(r.ref)
      });
    } else {
